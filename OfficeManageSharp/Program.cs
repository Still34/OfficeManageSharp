...
namespace OfficeManageSharp
{
    internal class Program
    {
        ...
        private static async Task ProcessDocumentsAsync(Options options)
        {
            ...
            var doSave = !options.IsSimulation;
            ...
            if (word.Length > 0)
            {
                ...
                if (options.ShouldMarkAsFinal)
                    await Task.WhenAll(word.Select(s => Task.Run(() => docManager.MarkDocxAsFinal(s, doSave))));

                if (options.ShouldRemoveEmbedFonts)
                    await Task.WhenAll(word.Select(s => Task.Run(() => docManager.RemoveDocxFonts(s, doSave))));
            }
            ...
            if (powerPoint.Length > 0)
            {
                ...
                if (options.ShouldMarkAsFinal)
                    await Task.WhenAll(powerPoint.Select(s => Task.Run(() => docManager.MarkPptxAsFinal(s, doSave))));
            }
            ...
            if (excel.Length > 0)
            {
                ...
                if (options.ShouldMarkAsFinal)
                    await Task.WhenAll(excel.Select(s => Task.Run(() => docManager.MarkXlsxAsFinal(s, doSave))));
            }
        }
